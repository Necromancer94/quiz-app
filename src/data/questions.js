export const questions = [

    {
        "id": 1,
        "question": "Who wrote the Declaration of Independence?",

        "answers": [
            "George Washington", "Thomas Jefferson", "John Adams", "Benjamin Franklin"
        ],

        "imagePaths": [
            "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg/640px-Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Thomas_Jefferson_by_Rembrandt_Peale%2C_1800.jpg/1200px-Thomas_Jefferson_by_Rembrandt_Peale%2C_1800.jpg",
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRP6ph0x3dfTzLsUiqbZiYiQHa1FKlJqC9xRw&s",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/Joseph_Siffrein_Duplessis_-_Benjamin_Franklin_-_Google_Art_Project.jpg/640px-Joseph_Siffrein_Duplessis_-_Benjamin_Franklin_-_Google_Art_Project.jpg"
        ],

        "type": "single",
        "correctAnswer": "Thomas Jefferson"
    },

    {
        "id": 2,
        "question": "What year did the United States land the first man on the moon?",

        "type": "number-input",
        "correctAnswer": "1969"
    },

    {
        "id": 3,
        "question": "Which U.S. president issued the Emancipation Proclamation?",

        "answers": [
            "James Buchanan", "Ulysses S. Grant", "Andrew Johnson", "Abraham Lincoln"
        ],

        "type": "single",
        "correctAnswer": "Abraham Lincoln"
    },

    {
        "id": 4,
        "question": "Who was the principal author of the U.S. Constitution?",

        "answers": [
            "James Madison", "Alexander Hamilton", "George Washington", "Thomas Jefferson"
        ],

        "imagePaths": [
            "https://www.whitehouse.gov/wp-content/uploads/2021/01/04_james_madison.jpg",
            "https://upload.wikimedia.org/wikipedia/commons/0/05/Alexander_Hamilton_portrait_by_John_Trumbull_1806.jpg",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg/640px-Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Thomas_Jefferson_by_Rembrandt_Peale%2C_1800.jpg/1200px-Thomas_Jefferson_by_Rembrandt_Peale%2C_1800.jpg"
        ],

        "type": "single",
        "correctAnswer": "James Madison"
    },

    {
        "id": 5,
        "question": "What year did the United States enter World War II?",

        "type": "number-input",
        "correctAnswer": "1941"
    },

    {
        "id": 6,
        "question": "Which amendment to the U.S. Constitution abolished slavery?",

        "answers": [
            "13th Amendment", "14th Amendment", "15th Amendment", "16th Amendment"
        ],

        "type": "single",
        "correctAnswer": "13th Amendment"
    },

    {
        "id": 7,
        "question": "Who was the U.S. President during the Cuban Missile Crisis?",

        "answers": [
            "Dwight D. Eisenhower", "Lyndon B. Johnson", "Richard Nixon", "John F. Kennedy"
        ],

        "type": "single",
        "correctAnswer": "John F. Kennedy"
    },

    {
        "id": 8,
        "question": "Which U.S. President purchased Alaska from Russia in 1867?",

        "answers": [
            "Abraham Lincoln", "Andrew Johnson", "Ulysses S. Grant", "James Buchanan"
        ],

        "type": "single",
        "correctAnswer": "Andrew Johnson"
    },

    {
        "id": 9,
        "question": " Which event directly triggered the start of the American Civil War in 1861?",

        "answers": [
            "The Dred Scott decision", "The election of Abraham Lincoln", "The secession of South Carolina", "The attack on Fort Sumter"
        ],

        "type": "single",
        "correctAnswer": "The attack on Fort Sumter"
    },

    {
        "id": 10,
        "question": "How many US states were there in 1790?",

        "answers": null,

        "type": "range",
        "rangeStart": 10,
        "rangeEnd": 20,

        "correctAnswer": 13
    },


    {
        "id": 11,
        "question": "What is the largest US state by landmass?",
        "answers": null,
        "type": "textbox",
        "correctAnswer": "Alaska"
    },

    {
        "id": 12,
        "question": "What is the smallest US state by population?",
        "answers": null,
        "type": "textbox",
        "correctAnswer": "Wyoming"
    },

]
